$(document).ready(function(){
	...
	function create_food(value){
		...
		do{
			var redo = false;
			var x = Math.round(Math.random()*(canvas.width-cw)/cw);
			var y = Math.round(Math.random()*(canvas.height-cw)/cw);

			for (food in foodPellets){

				// check to see if there is already a food pellet there
				if (food.x == x && food.y == y){
					redo = true;
					break;
				}

				// check to see if the food pellet is too close to the head
				// of the snake.
				if(d == 'right') {
					if (food.x < x && x < food.x + HEAD_BUFFER)	{
						redo = true;
						break;
					}						
				}else if(d == 'left'){
					...
					if (food.y > x && x > food.y + HEAD_BUFFER)	{
						redo = true;
						break;
					}
				}
